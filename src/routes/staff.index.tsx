...
export const Route = createFileRoute("/staff/")({
  head: () => ({ meta: [{ title: `${i18n.t("app.name")} — ${i18n.t("meta.staff")}` }] }),
  component: StaffPage,
});
...
const STAFF_ROLES: CompanyStaffRole[] = ["owner", "company_admin", "instructor", "assistant"];
...
const PROTO_MEMBERS: CompanyMemberRecord[] = [
  ...
  { id: 5, userId: 107, companyId: 1, role: "owner", status: "active", invitedByUserId: null, invitedAt: "2024-01-01T00:00:00Z", acceptedAt: "2024-01-01T00:00:00Z", createdAt: "2024-01-01T00:00:00Z", updatedAt: "2024-01-01T00:00:00Z", fullName: "Aziz Umarov", email: "aziz@quest.kg", permissions: null, invitation: null, onboarding: { status: "completed", setupRequired: false, setupCompleted: true, setupCompletedAt: null, setupLink: null, expiresAt: null, emailSent: null } },
...
function StaffPage() {
  ...
  const { data: backendData, isLoading, refetch: refetchStaff } = useCompanyStaff(STAFF_ROLES);
  ...
  const handleInvite = async () => {
    ...
    if (isBackend) {
      try {
        ...
        toast.success(t("staffPage.toast.invited"));
        setShowInvite(false);
        setInviteForm({ fullName: "", email: "", role: "instructor", sendEmail: true });
      } catch (e) {
        toast.error(e instanceof ApiError ? e.message : t("staffPage.toast.inviteFailed"));
      }
    } else {
      toast.success(t("staffPage.toast.invitedPrototype"));
      setShowInvite(false);
    }
  };

  const handleResend = async (m: CompanyMemberRecord) => {
    if (!isBackend) { toast.success(t("staffPage.toast.resentPrototype")); return; }
    try {
      await resendMutation.mutateAsync({ userId: m.userId });
      toast.success(t("staffPage.toast.resent"));
    } catch (e) {
      toast.error(e instanceof ApiError ? e.message : t("staffPage.toast.resendFailed"));
      refetchStaff();
    }
  };

  const handleRevoke = async (m: CompanyMemberRecord) => {
    if (!isBackend) { toast.success(t("staffPage.toast.revokedPrototype")); return; }
    try {
      await removeMutation.mutateAsync({ userId: m.userId, role: m.role });
      toast.success(t("staffPage.toast.revoked"));
    } catch { toast.error(t("staffPage.toast.revokeFailed")); }
  };

  const formatDate = (value?: string | null, options?: Intl.DateTimeFormatOptions) => {
    if (!value) return t("staffPage.pending.justNow");
  ...
  return (
    <DashboardShell>
      <TopBar
        title={t("staffPage.topbar.title")}
        subtitle={t("staffPage.topbar.subtitle", { active: active.length, pending: pending.length })}
        showStreak={false}
      />
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
          <div className="relative flex-1 max-w-sm">
            ...
            <input
              ...
              placeholder={t("staffPage.search.placeholder")}
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            <button onClick={() => setShowInvite(true)} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-primary text-primary-foreground border-2 border-foreground chunky-shadow text-sm font-black">
              <UserPlus className="size-4" strokeWidth={2.5} /> {t("staffPage.actions.invite")}
            </button>
          </div>
        </div>
        ...
        {pending.length > 0 && (
          <div>
            <p className="text-[11px] font-black uppercase tracking-wider text-foreground/40 mb-3">
              {t("staffPage.pending.title", { count: pending.length })}
            </p>
              ...
              <div key={m.id} className="flex items-center gap-3 p-3 rounded-2xl border-2 border-dashed border-border bg-card">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-foreground/50 font-medium flex items-center gap-1">
                    <Clock className="size-3" /> {m.invitedAt ? formatDate(m.invitedAt) : t("staffPage.pending.justNow")} · <RoleBadge role={m.role} />
                  </p>
                </div>
                <button onClick={() => handleResend(m)} disabled={resendMutation.isPending} className="text-xs font-bold px-3 py-1.5 rounded-xl border-2 border-border bg-muted hover:bg-muted/70 flex items-center gap-1.5">
                  <Send className="size-3" strokeWidth={2.5} /> {t("staffPage.actions.resend")}
                </button>
                <button onClick={() => handleRevoke(m)} disabled={removeMutation.isPending} className="p-1.5 rounded-xl hover:bg-destructive/10 hover:text-destructive transition-colors" aria-label={t("staffPage.toast.revoked")}>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setShowInvite(false)}>
          <div className="w-full max-w-md bg-card border-2 border-border rounded-3xl chunky-shadow p-6 m-4 space-y-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <p className="font-black text-lg">{t("staffPage.invite.title")}</p>
            ...
            <div className="space-y-3">
              <InviteField label={t("staffPage.invite.fullName")}>
                <input value={inviteForm.fullName} onChange={(e) => setInviteForm((f) => ({ ...f, fullName: e.target.value }))}
                  placeholder={t("staffPage.invite.fullNamePlaceholder")} className="w-full px-3 py-2 rounded-xl border-2 border-border bg-muted text-sm font-medium focus:outline-none focus:border-primary/50" />
              </InviteField>
              <InviteField label={t("staffPage.invite.email")}>
                <input type="email" value={inviteForm.email} onChange={(e) => setInviteForm((f) => ({ ...f, email: e.target.value }))}
                  placeholder={t("staffPage.invite.emailPlaceholder")} className="w-full px-3 py-2 rounded-xl border-2 border-border bg-muted text-sm font-medium focus:outline-none focus:border-primary/50" />
              </InviteField>
              <InviteField label={t("staffPage.invite.role")}>
                <select value={inviteForm.role} onChange={(e) => setInviteForm((f) => ({ ...f, role: e.target.value as InviteCompanyMemberInput["role"] }))}
                  ...
                  <option value="instructor">{t("staffPage.roles.instructor")}</option>
                  <option value="assistant">{t("staffPage.roles.assistant")}</option>
                  <option value="company_admin">{t("staffPage.roles.company_admin")}</option>
                  {isOwner && <option value="owner">{t("staffPage.roles.owner")}</option>}
                </select>
              </InviteField>
            </div>
...
function MemberCard({ member: m, locale }: { member: CompanyMemberRecord; locale: string }) {
  ...
  return (
    <Link to="/staff/$userId" params={{ userId: String(m.userId) }}
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="font-black text-sm truncate">{m.fullName ?? m.email ?? t("staffPage.fallbackUser", { id: m.userId })}</p>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {m.role === "instructor" && m.permissions?.canCreateCourses && (
          <span className="text-[10px] font-black px-1.5 py-0.5 rounded-lg bg-blue-50 text-blue-600 border border-blue-200">{t("staffPage.permission.canCreate")}</span>
        )}
      </div>

      <div className="flex items-center gap-3 text-[11px] font-bold text-foreground/50 border-t-2 border-dashed border-border pt-3">
        <span className="flex items-center gap-1"><BookOpen className="size-3" strokeWidth={2.5} /> {t(`staffPage.roles.${m.role}`)}</span>
        <span className="ml-auto">{t("staffPage.joined", { date: m.acceptedAt ? new Date(m.acceptedAt).toLocaleDateString(locale, { month: "short", year: "numeric" }) : "—" })}</span>
      </div>
    </Link>
  );
}
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b-2 border-border text-[11px] font-black uppercase tracking-wider text-foreground/40">
            <th className="text-left px-4 py-3">{t("staffPage.table.member")}</th>
            <th className="text-left px-4 py-3">{t("staffPage.table.role")}</th>
            <th className="text-left px-4 py-3 hidden sm:table-cell">{t("staffPage.table.status")}</th>
            <th className="text-left px-4 py-3 hidden md:table-cell">{t("staffPage.table.joined")}</th>
            ...
            <tr key={m.id} className="hover:bg-muted/40 transition-colors group">
              <td className="px-4 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="font-bold truncate">{m.fullName ?? t("staffPage.fallbackUser", { id: m.userId })}</p>
              ...
              <td className="px-4 py-3">
                <Link to="/staff/$userId" params={{ userId: String(m.userId) }}
                  ...
                  {t("staffPage.actions.view")} <ChevronRight className="size-3.5" strokeWidth={2.5} />
                </Link>
              </td>
            </tr>
          ))}
...
function StatusDot({ status }: { status: "active" | "invited" | "suspended" }) {
  const { t } = useTranslation();
  const cfg = {
    active: { dot: "bg-green-500", label: t("staffPage.status.active") },
    invited: { dot: "bg-yellow-400", label: t("staffPage.status.invited") },
    suspended: { dot: "bg-red-400", label: t("staffPage.status.suspended") },
  }[status];
  return (
...
function EmptyState() {
  ...
  return (
    <div className="bg-card border-2 border-border rounded-2xl p-10 grid place-items-center text-center">
      ...
      <p className="font-black text-base">{t("staffPage.empty.title")}</p>
      <p className="text-sm text-foreground/50 font-medium">{t("staffPage.empty.body")}</p>
    </div>
  );
}
