...
function OnboardingPage() {
  ...
  const [brandColor, setBrandColor] = useState<string>(BRAND_COLORS[0].value);
  const [invites, setInvites] = useState<Invite[]>([
    { id: crypto.randomUUID(), email: "", role: "company_admin" },
  ]);

  const canNext = useMemo(() => {
    if (stepId === "organization") {
      return orgName.trim().length >= 2 && /^[a-z0-9-]{3,32}$/.test(subdomain);
    }
    if (stepId === "branding") return Boolean(brandColor);
    if (stepId === "invites") {
      return invites.every(
        (i) => i.email === "" || /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(i.email)
      );
    }
    return true;
      ...
      <header className="border-b border-border bg-card">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="size-9 bg-secondary text-secondary-foreground rounded-lg grid place-items-center font-black italic">
              Q
            </div>
    ...
    <div className="text-center space-y-6">
      ...
      <div className="space-y-3">
        ...
        <p className="text-muted-foreground text-base sm:text-lg max-w-xl mx-auto">
          Let's set up your academy in a few quick steps — name it, brand it,
          and invite your team. You can change everything later.
        </p>
      </div>
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          This is how your school appears to students and instructors.
        </p>
      </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-stretch rounded-lg border border-input bg-background focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2 focus-within:ring-offset-background overflow-hidden">
            <Input
              ...
              maxLength={32}
            />
            <div className="px-3 grid place-items-center text-sm text-muted-foreground bg-muted/50 border-l border-border">
              .questlms.app
            </div>
          </div>
          <p
            className={
              "text-xs " +
              (subdomainOk ? "text-muted-foreground" : "text-destructive")
            }
          >
            {subdomainOk
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Upload a logo and pick a brand color. Students will see this across
          your portal.
        </p>
      </div>
            ...
            {BRAND_COLORS.map((c) => {
              ...
              return (
                <button
                  ...
                  className={
                    "aspect-square rounded-lg border-2 transition-all grid place-items-center " +
                    (selected
            ...
            <div className="flex items-center gap-3">
              {logoDataUrl ? (
                <img
                  src={logoDataUrl}
                  alt=""
                  className="size-9 rounded-lg bg-white/95 object-contain p-1"
                />
              ) : (
...
function InvitesStep({
  ...
  function add() {
    setInvites([
      ...invites,
      { id: crypto.randomUUID(), email: "", role: "instructor" },
    ]);
  }
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Add admins, instructors, or assistants. They'll get an email to join.
            ...
            >
              <div className="flex-1">
                <Input
                  value={inv.email}
                  onChange={(e) =>
                    update(inv.id, { email: e.target.value.slice(0, 254) })
                  }
                  placeholder="teammate@school.com"
                  type="email"
                  maxLength={254}
                  aria-invalid={!valid}
              ...
              <div className="w-full sm:w-48">
                <Select
                  value={inv.role}
                  onValueChange={(v) =>
                    update(inv.id, { role: v as Invite["role"] })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <Button
                ...
                disabled={invites.length === 1 && idx === 0}
    ...
    <div className="text-center space-y-6">
      ...
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">
          Your academy is ready at{" "}
          <span className="font-mono text-foreground">
            {subdomain}.questlms.app
          </span>
          .
        </p>
      </div>
      ...
      <Card className="p-5 max-w-md mx-auto text-left space-y-3">
        ...
        <ul className="space-y-2 text-sm">
          ...
          <li className="flex items-start gap-2">
            ...
            <span>
              {invites.length > 0
                ? `${invites.length} invite${invites.length === 1 ? "" : "s"} will be sent.`
                : "Invite your team from the Staff page."}
            </span>
          </li>
          <li className="flex items-start gap-2">
            <Check className="size-4 text-primary mt-0.5 shrink-0" />
